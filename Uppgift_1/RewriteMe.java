...
public class RewriteMe {
    ...
    public List<String> getAllQuestionStringsBelongingACategory(Category category){
       return questions.stream().filter(s -> s.getCategory().equals(category)).map(Question::getQuestionString)
                .collect(Collectors.toList());
    }
    ...
    public List<String> getAllAnswerOptionsDistinct(){
       return questions.stream().flatMap(s -> s.getAllAnswers().stream()).distinct().toList();
    }
    ...
    public Map<Category, List<String>> getQuestionGroupedByCategory(){
       ...
       /* return questions.stream().map(Question::getCategory)
                .collect(Collectors.groupingBy());
        .collect(Collectors.toMap(k -> k.getCategory(),
                        v -> v.getQuestionString()));*/

       /* categoryMap.forEach((k,v) -> System.out.println(k + " " + v));

        System.out.println(categoryMap.entrySet());
    ...
    {
        List<String> answers = questions.stream().filter(n -> n.getCategory().equals(c))
                .flatMap(s -> s.getAllAnswers().stream()).distinct().collect(Collectors.toList());
        Optional<String> longestName = answers.stream()
                .reduce((answer1, answer2)->answer1.length() >= answer2.length() ? answer1 : answer2);
        String longest = longestName.get();
        return longest;
    }
